                        ...
                        <div class="tab-pane fade show active" id="content" role="tabpanel" aria-labelledby="content-tab">
                            <div class="row">
                                ...
                                <div class="col-sm-12 col-md-9 pt-3">
                                    ...
                                    <select name="parent_id" class="form-control">
                                        ...
                                        @foreach ($topic_menus->where('section', 3) as $main_menu)
                                            @if (count($main_menu->appearedChildren) == false)
                                                <option style="color: black;font-weight: bold;font-size:18px;"
                                                    value="{{ $main_menu->id }}"
                                                    {{ old('parent_id', $topicsMenu->parent_id) == $main_menu->id ? 'selected' : null }}>
                                                    {{ $main_menu->title }}
                                                </option>
                                            @else
                                                <option style="color: black;font-weight: bold;font-size:18px;"
                                                    ...
                                                    @foreach ($main_menu->appearedChildren as $sub_menu)
                                                        @if (count($sub_menu->appearedChildren) == false)
                                                            <option style="color:blue;font-weight:bold;font-size:15px;"
                                                                value="{{ $sub_menu->id }}"
                                                                {{ old('parent_id', $topicsMenu->parent_id) == $sub_menu->id ? 'selected' : null }}>
                                                                &nbsp; &nbsp; &nbsp;{{ $sub_menu->title }}
                                                            </option>
                                                        @else
                                                            <option style="color:blue;font-weight:bold;font-size:15px;"
                                                            ...
                                                            @if ($sub_menu->appearedChildren !== null && count($sub_menu->appearedChildren) > 0)
                                                                @foreach ($sub_menu->appearedChildren as $sub_menu_2)
                                                                    <option style="font-size: 14px;"
                                                                        value="{{ $sub_menu_2->id }}"
                                                                        {{ old('parent_id', $topicsMenu->parent_id) == $sub_menu_2->id ? 'selected' : null }}>
                                                                        &nbsp; &nbsp; &nbsp; &nbsp; &nbsp;
                                                                        &nbsp;{{ $sub_menu_2->title }}
                                                                    </option>
                                                                @endforeach
                            ...
                            @foreach (config('locales.languages') as $key => $val)
                                <div class="row ">
                                    ...
                                    <div class="col-sm-12 col-md-9 pt-3">
                                        <textarea id="tinymceExample" name="description[{{ $key }}]" rows="10" class="form-control ">{!! old('description.' . $key, $topicsMenu->getTranslation('description', $key)) !!}</textarea>
                                ...
                                <div class="col-sm-12 col-md-9 pt-3">
                                    <div class="input-group iconpicker-container ">
                                        <input data-placement="bottomRight"
                                            ...
                                            value=" {{ old('icon', $topicsMenu->icon) ?? 'fas fa-archive' }}"
                                            type="text" name="icon">
                                        <span class="input-group-addon btn btn-primary">
                                            <i class="{{ $topicsMenu->icon ?? 'fas fa-archive' }}"></i>
                                        </span>
                                    </div>
                                    @error('icon')
                                        <span class="text-danger">{{ $message }}</span>
                                ...
                                <div class="col-sm-12 col-md-9 pt-3">
                                    <div class="input-group flatpickr" id="flatpickr-datetime">
                                        <input type="text" name="published_on" class="form-control"
                                            ...
                                            value="{{ old('published_on', $topicsMenu->published_on ? \Carbon\Carbon::parse($topicsMenu->published_on)->format('Y/m/d h:i A') : '') }}">
                                ...
                                <div class="col-sm-12 col-md-9 pt-3">
                                    <div class="form-check form-check-inline">
                                        <input type="radio" class="form-check-input" name="status" id="status_active"
                                            value="1"
                                            {{ old('status', $topicsMenu->status) == '1' ? 'checked' : '' }}>
                                    ...
                                    <div class="form-check form-check-inline">
                                        <input type="radio" class="form-check-input" name="status"
                                            ...
                                            {{ old('status', $topicsMenu->status) == '0' ? 'checked' : '' }}>
                                ...
                                <div class="row">
                                    ...
                                    <div class="col-sm-12 col-md-9 pt-3">
                                        <input type="text" name="metadata_title[{{ $key }}]"
                                            ...
                                            value="{{ old('metadata_title.' . $key, $topicsMenu->getTranslation('metadata_title', $key)) }}"
                                            class="form-control">
                                ...
                                <div class="row">
                                    ...
                                    <div class="col-sm-12 col-md-9 pt-3">
                                        <input type="text" name="metadata_description[{{ $key }}]"
                                            ...
                                            value="{{ old('metadata_description.' . $key, $topicsMenu->getTranslation('metadata_description', $key)) }}"
                                            class="form-control">
                                            ...
                                            <div class="card-body">
                                                <div>
                                                    <input name="metadata_keywords[{{ $key }}]"
                                                        ...
                                                        value="{{ old('metadata_keywords.' . $key, $topicsMenu->getTranslation('metadata_keywords', $key)) }}" />
                                                </div>
                                            </div>
                                        </div>
