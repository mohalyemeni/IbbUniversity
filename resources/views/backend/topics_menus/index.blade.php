                ...
                <div class="card-body">
                    ...
                    <div class="table-responsive">
                        <table class="table table-hover table-striped table-bordered dt-responsive nowrap"
                            ...
                            <tbody>
                                @forelse ($topics_menus as $topics_menu)
                                    <tr>
                                        <td>
                                            {{ $topics_menu->title }}
                                            <br>
                                            @if ($topics_menu->parent != null)
                                                <small
                                                    style="background: #17a2b8;color:white;padding:1px 3px;border-radius: 5px; font-size:11px">
                                                    {{-- تابع للقائمة: --}}
                                                    <span>{{ $topics_menu->parent?->title }}</span>
                                                </small>
                                            @endif
                                        </td>
                                        ...
                                        <td class="d-none d-sm-table-cell">
                                            {{ $topics_menu->published_on->diffForHumans() }}
                                        </td>
                                        <td>
                                                ...
                                                <div class="dropdown mb-2 ">
                                                    ...
                                                    <div class="dropdown-menu" aria-labelledby="dropdownMenuButton">
                                                        <a class="dropdown-item d-flex align-items-center"
                                                            href="{{ route('admin.topics_menus.edit', $topics_menu->id) }}">
                                                            <i data-feather="edit-2" class="icon-sm me-2"></i>
                                                            <span class="">{{ __('panel.operation_edit') }}</span>
                                                        </a>

                                                        <a href="javascript:void(0);"
                                                            onclick="confirmDelete('delete-topics_menu-{{ $topics_menu->id }}', '{{ __('panel.confirm_delete_message') }}', '{{ __('panel.yes_delete') }}', '{{ __('panel.cancel') }}')"
                                                            class="dropdown-item d-flex align-items-center">
                                                            <i data-feather="trash" class="icon-sm me-2"></i>
                                                            <span class="">{{ __('panel.operation_delete') }}</span>
                                                        </a>
                                                        <form
                                                            action="{{ route('admin.topics_menus.destroy', $topics_menu->id) }}"
                                                            method="post" class="d-none"
                                                            id="delete-topics_menu-{{ $topics_menu->id }}">
                                                            @csrf
                                                            @method('DELETE')
                                                        </form>
                                                        ...
                                                        <a href="javascript:void(0);"
                                                            ...
                                                            data-copy-text="{{ config('app.url') }}/topics_menus/{{ $topics_menu->slug }}"
                                                            data-id="{{ $topics_menu->id }}" title="Copy the link">
                                                    ...
                                                    <span class="copyMessage" data-id="{{ $topics_menu->id }}"
                                                        style="display:none;">
                                                        {{ __('panel.copied') }}
                                                    </span>
                                                </div>
